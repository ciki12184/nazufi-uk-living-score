...
def find_field(row, aliases):
    n = {normalise_key(k): v for k, v in row.items()}
    for alias in aliases:
        if alias in n:
            return clean(n[alias])
    return ""
...
def ingest():
    ...
    existing = con.execute(
        "SELECT id FROM bulk_dataset_files WHERE source_key='ofsted_schools' AND sha256=? AND is_published=1",
        (digest,)
    ).fetchone()
    if existing:
        con.close()
    ...
    try:
        ...
        for row in reader:
            ...
            name = find_field(row, ["school_name", "provider_name", "establishment_name"])
...
if __name__ == "__main__":
    ...
    # Transactional publication: new data replace old only after full validation.
    con.execute("BEGIN")
    try:
        con.execute("DELETE FROM schools")
        con.executemany("""
        ...
        con.close()
    return {"status":"published","rows":len(staged),"sha256":digest,"label":info["label"]}
